...
No unit test suite exists yet. `scripts/booking-tests/run.mjs` is an integration harness for the booking Edge Functions and RPCs; it writes to the live project with three dedicated test accounts, so read `scripts/booking-tests/README.md` and only run it with the user's explicit go-ahead.
...
- Supabase Edge Functions live under `supabase/functions/`; function source, `supabase/config.toml`, and `supabase/migrations/` are repo source and should be versioned. Shared function code lives in `supabase/functions/_shared/`: `http.ts` (`servePost` for CORS/method/auth-header/JSON handling, `jsonResponse`, `readString`), `supabase.ts` (`authenticate` for the caller-scoped client plus session check, `createAdminClient`, `logDatabaseError`), `errors.ts` (declarative `ErrorRule` lists mapped by `databaseErrorResponse`), and `reservations.ts` (shared booking input parsing and reservation error rules). New functions should build on these instead of copying the boilerplate.
...
Admins and professors review pending join requests in the `Join Requests` tab of `components/dashboard/SchoolManagementTabs.tsx`. The list is loaded through `get_school_join_requests_with_profiles`, which returns request id, user id, profile name, email, and request time for school admins and professors (admin or professor check inlined in the RPC, migration `20260925064502_professor_join_request_review.sql`; the admin-only `private.is_school_admin` and `JoinRequests` RLS are unchanged). Review submission calls the deployed Supabase Edge Function `review-school-join-requests`, which verifies the caller is an admin or professor, adds approved users to `SchoolMembers` with role `student`, and deletes processed `JoinRequests` rows.
...
The active panel is client `useState` in `ScheduleClient.tsx`, initialized once from `?panel=`, and the URL is kept in sync with `window.history.replaceState` — never `router.replace`. The server never reads `panel`, so routing a panel switch would refetch the entire dynamic page (`auth.getUser()` plus every Supabase query) for output that does not change, which made each switch take about a second. Any future client-only URL state on this page must follow the same rule: `history.replaceState` for params the server does not consume, `router` navigation only when the server output actually depends on the param.

`app/dashboard/schools/[schoolId]/page.tsx` is the school management shell for admins, professors, and exam supervisors. It verifies the signed-in user's `SchoolMembers` row or `Schools.created_by` ownership for the selected active school before rendering. Admins can manage members, invites, join requests, and settings; professors can view members, search the member list, review join requests, schedule exams for students, and cancel school reservations; exam supervisors can view reservations and use attendance only.
...
`app/dashboard/layout.tsx` is a data-free server layout whose only job is mounting `components/release/WhatsNewModal.tsx`, so the release dialog covers `/dashboard`, `/dashboard/schedule`, and `/dashboard/schools/[schoolId]` from one place. It deliberately does not cover `/`, `/login`, or `/invite/[inviteToken]` — a release dialog must not interrupt signing in or accepting an invite. Do not move authorization or data fetching into this layout; each route still builds its own shell.

**To announce a release, prepend an entry to `RELEASES` in `lib/release-notes.ts`** in the same PR as the feature. Array position is the only ordering authority; the `YYYY.MM.N` version string is compared by equality and never parsed. Item icons come from the closed `RELEASE_ICONS` map, so an unknown icon name is a type error.

This is intentionally separate from `UserNotifications` / `NotificationBell`, which is a per-user event feed (one row per user per event). A release announcement is the same content for everyone shown once on entry, so it lives in code and needs no database rows, migration, or RLS change.

Seen state is localStorage under `js-release-seen:v1`, read through `useSyncExternalStore` in `lib/release-seen.ts` for the same reason the theme is (`lib/theme-store.ts`): the server snapshot renders nothing, which keeps hydration clean without a setState-in-effect dance. Every localStorage access for release notes lives in that one module, so moving to a `Profiles` column later is a single-file change. Known tradeoff: state is per-device and per-browser, so a release shows once per browser and is lost when site data is cleared.

Behavior rules: a user with no marker sees only the latest release, not the whole history; dismissing records the newest version so catching up on several releases is one interaction; dismissal is final (there is no re-open control); and when localStorage is blocked the dialog does not render at all, since it could otherwise never be dismissed for good.

`components/release/ReleaseNotesDialog.tsx` is presentation only and takes `releases` plus `onDismiss`. It follows the hand-rolled dialog pattern from `MemberKickDialog` (portal, `var(--overlay-scrim)`, `.panel`, `var(--shadow-dialog)`, `role="dialog"`), and owns focus restore, focus trap, Escape, body scroll lock, and `overscroll-behavior: contain`.
...
- `supabase/migrations/20260924104243_refactor_booking_rpc_helpers.sql` moves the shared booking logic of `reserve_exam_slot`, `schedule_exam_for_student`, `update_reservation`, `create_schedule_request`, `review_schedule_request`, and `mark_schedule_request_teacher_seen` into private helpers (`assert_exam_fields`, `assert_booking_date`, `school_today`, `caller_staff_role`, `lock_slot_date`, `count_confirmed_reservations`, `get_active_primary_slot`, `get_active_overflow_slot`, `has_slot_group_reservation`, `claim_slot_group_seat`, `resolve_schedule_request`) without changing public signatures. New booking RPCs should use these helpers instead of copying the checks.
- `supabase/migrations/20260924104425_fix_booking_date_and_staff_overflow.sql` makes every booking RPC use the school-timezone "today" and makes `schedule_exam_for_student` route from a full main room into its overflow room.
- `supabase/migrations/20260925063108_drop_unused_functions.sql` drops six functions nothing called: `create_overflow_slot`, `set_slot_active`, `update_slot_capacity` (all superseded by `create_overflow_exam_slot` / `update_exam_slot`), `get_school_behaviour_indexes`, `get_student_reservation_history`, and `private.is_student_school_member`.

Note: the live database also contains the overflow-room migrations (`20260603…`) that are no longer in this folder on `main`; the live function definitions are the source of truth until that drift is reconciled.
...
The full recovery plan is documented in `docs/notes/free-database-backup-and-recovery.md`. Layers 1, 2, and 4 are implemented; Layers 3, 5, and 6 are not yet implemented.
